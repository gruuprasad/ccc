...
TEST_CASE("Fast Parser: IF-ELSE statement test"){
    PARSE_VALID_STATEMENT("if (\"true\") ; ") PARSE_VALID_STATEMENT(
        "if (1) { if (2) return; else return; } else return;")
        PARSE_VALID_STATEMENT(
            "if (1) { if (2) return; else return; }"
            " else { if (2) return;}") PARSE_VALID_STATEMENT("if (1) if (2) ; "
                                                             "else ;")
            PARSE_INVALID_STATEMENT("if (\"true\")  ") PARSE_INVALID_STATEMENT(
                "if (1)  else ;") PARSE_INVALID_STATEMENT("if (1) ; else ")
                PARSE_INVALID_STATEMENT(
                    "if (1) { if (2) return; else return;  else return;")
                    PARSE_INVALID_STATEMENT(
                        "if (1 { if (2) return; else return; } else return;")
                        PARSE_INVALID_STATEMENT(
                            ...
                            PARSE_INVALID_STATEMENT(
                                "if (1 { else return; } else return;")}

TEST_CASE("Fast Parser: Jump statement"){
    PARSE_VALID_STATEMENT("{ goto a; }") PARSE_VALID_STATEMENT("{ continue; }")
        PARSE_VALID_STATEMENT("{ break; }") PARSE_VALID_STATEMENT("{ return; }")
