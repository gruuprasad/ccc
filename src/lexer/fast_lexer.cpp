...
void print_token_extra(std::string format, char *filename, long line, long column, const std::string &extra) {
  std::printf(&format[0], filename, line, column + 1, &extra[0]);
}

void print_token(std::string format, char *filename, long line, long column) {
  std::printf(&format[0], filename, line, column + 1);
}

template<typename... Args>
void dummy(Args... args __attribute__((unused))) {
}
...
//  this->tokenize = tokenize;
  if (tokenize) {
    this->print_ptr = &print_token;
    this->print_extra_ptr = &print_token_extra;
  } else {
    this->print_ptr = &dummy;
    this->print_extra_ptr = &dummy;
  }
...
inline bool FastLexer::munchNumber() {
  ...
  (*this->print_extra_ptr)("%s:%ld:%ld: constant %s\n",
                           filename,
                           line,
                           column,
                           std::string(&content[oldPosition], position - oldPosition));
...
inline bool FastLexer::munchIdentifier() {
  ...
  token_list.emplace_back(TokenType::IDENTIFIER,
                          ...
                          column,
                          std::string(&content[oldPosition], position - oldPosition));
  (*this->print_extra_ptr)("%s:%ld:%ld: identifier %s\n",
                           filename,
                           line,
                           column,
                           std::string(&content[oldPosition], position - oldPosition));
...
inline bool FastLexer::munchString() {
  ...
  (*this->print_extra_ptr)("%s:%ld:%ld: string-literal \"%s\"\n",
                           filename,
                           line,
                           initColumn,
                           std::string(&content[oldPosition + 1], position - oldPosition - 1));
