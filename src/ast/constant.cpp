...
std::string IntegerConstant::toGraphWalker() {
  std::stringstream ss;
  ss << this->id << "[label=\"" << this->constant << "\" shape=diamond style=filled fillcolor=lightyellow];\n";
  return ss.str();
}
...
EnumerationConstant::EnumerationConstant(int id, std::string &constant) : Constant(id) {
  this->constant = constant;
}
